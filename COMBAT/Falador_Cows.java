...
@ScriptManifest(author="Acinate", name="Cow Meleer", info="Kills Cows South Falador", logo="", version=1.0)
public class Falador_Cows extends Script {
    ...
    public void onStart() {
        ...
        this.startXP = this.skills.getExperience(Skill.ATTACK) + this.skills.getExperience(Skill.STRENGTH) +
                + this.skills.getExperience(Skill.DEFENCE) + this.skills.getExperience(Skill.DEFENCE);
            ...
            case ATTACK:
                ...
                if (nearestCow != null) {
                    ...
                    if (!myPlayer().isAnimating() && !myPlayer().isMoving() && myPlayer().getInteracting() == null) {
                        if (nearestCow.isVisible() && !nearestCow.isUnderAttack() && !nearestCow.isAnimating()) {
                            status = "Attacking Cow";
                            nearestCow.interact("Attack");
                            for (int x = 0; x < 100 && (!myPlayer().isAnimating() || !myPlayer().isMoving()); x++) {
                                sleep(20);
                            }
                        } else if (!nearestCow.isVisible()) camera.toEntity(nearestCow);
                    }
                } else if (NearestCowCalf != null) {
                    status = "Attacking Nearest Cow Calf";
                    if (!myPlayer().isAnimating() && !myPlayer().isMoving() && myPlayer().getInteracting() == null) {
                        if (NearestCowCalf.isVisible() && !NearestCowCalf.isUnderAttack() && !NearestCowCalf.isAnimating()) {
                            ...
                            antiBan();
                        } else if (!NearestCowCalf.isVisible()) camera.toEntity(NearestCowCalf);
                    }
                } else if (cow != null) {
                    status = "Attacking Cow";
                    if (!myPlayer().isAnimating() && !myPlayer().isMoving() && myPlayer().getInteracting() == null) {
    ...
    public void antiBan() throws InterruptedException {
        ...
        if (random < 2) {
            ...
            tabs.getSkills().hoverSkill(Skill.ATTACK);
        }
        else if (random == 3) {
            status = "Checking Magic XP";
            tabs.getSkills().hoverSkill(Skill.STRENGTH);
        }
        else if (random == 4) {
    ...
    public void onPaint(Graphics2D g) {
        ...
        this.currentXP = this.skills.getExperience(Skill.ATTACK) + this.skills.getExperience(Skill.STRENGTH) +
                + this.skills.getExperience(Skill.DEFENCE) + this.skills.getExperience(Skill.DEFENCE);
